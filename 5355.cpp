...
int apr[N],coapr[N];
vector<Ask>sp[N];
IL bool cmp(const Ask& a,const Ask& b)
{
	return a.x<b.x;
}
int main()
{
	...
	for(int i=1;i<=m;i++)
	{
		...
		else if(op==4)
		{
			if(x==0)
			{
				ans[q[i].id]=(s[0]&&s[0]!=s.count());
			}
			else if(x>=sq)
			{
				for(int j=1;j*x<=n;j++)
				{
					if(s[j]&&s[static_cast<size_t>(j)*x])
					{
						ans[q[i].id]=1;
						break;
					}
			...
			{
				sp[q[i].x].push_back(q[i]);
			}
		}
	}
	for(int x=1;x<=sq;x++)
	{
		if(sp[x].size()==0) continue;
		...
		for(int i=1;i<=100000;i++)
		{
			...
			if(x*a[i]<=100000)
			{
				now=max(now,apr[x*a[i]]);
			}
			if(a[i]%x==0)
			{
				now=max(now,apr[a[i]/x]);
			}
			coapr[i]=now;
		}
		for(int i=0;i<sp[x].size();i++)
		{
			if(sp[x][i].l<=coapr[sp[x][i].r]) ans[sp[x][i].id]=1;
		}
	}
