...
namespace SAM
{
	...
	struct state
	{
		...
		state operator=(const state& z)
		{
			len=z.len;
			link=z.link;
			for(int i=0;i<30;i++) nxt[i]=z.nxt[i];
			f=0;
			return *this;
		}
...
void top()
{
	...
	for(;l<=r;l++)
	{
		for(int j=0;j<30;j++)
		{
			...
			{
				in[st[q[l]].nxt[j]]--;
				if(in[st[q[l]].nxt[j]]==0) q[++r]=st[q[l]].nxt[j];
			}
		}
	}
	for(int i=r;i>=1;i--)
	...
	for(int i=r;i>=1;i--)
	{
		...
		for(int j=0;j<30;j++)
		{
			if(st[q[i]].nxt[j]) sum[q[i]]+=sum[st[q[i]].nxt[j]];
		}
	}
}
void ask(int k)
{
	...
	while(k>0)
	{
		...
		for(j=0;j<30;j++)
		{
			if(st[x].nxt[j])
			{
				if(k>sum[st[x].nxt[j]]) k-=sum[st[x].nxt[j]];
				else break;
			}
		}
		putchar(j+'a');
